...
Request.prototype.init = function (options) {
    ...
    if (self.proxy) {
        ...
        if (http.globalAgent && self.uri.protocol === "https:") {
            ...
            var tunnelOptions = { proxy:{ host:self.proxy.hostname, port:+self.proxy.port, proxyAuth:self.proxy.auth }, ca:this.ca }
...
Request.prototype._updateProtocol = function () {
    ...
    if (protocol === 'https:') {
        ...
        if (self.proxy) {
            ...
            var tunnelFn = self.proxy.protocol === 'http:'
                ? tunnel.httpsOverHttp : tunnel.httpsOverHttps
            var tunnelOptions = { proxy:{ host:self.proxy.hostname, post:+self.proxy.port, proxyAuth:self.proxy.auth }, ca:self.ca }
            self.agent = tunnelFn(tunnelOptions)
            return
        }
...
Request.prototype.start = function () {
    ...
    self.req = self.httpModule.request(self, function (response) {
        ...
        if (self.httpModule === https &&
            self.strictSSL && !response.client.authorized) {
        ...
        if (response.statusCode >= 300 && response.statusCode < 400 &&
            ...
            self.redirects.push(
                { statusCode:response.statusCode, redirectUri:response.headers.location
                }
            )
...
Request.prototype.aws = function (opts, now) {
    ...
    this.setHeader('authorization', aws.authorization(
        { key:opts.key, secret:opts.secret, verb:this.method, date:date, resource:aws.canonicalizeResource('/' + opts.bucket + this.path), contentType:this.headers['content-type'] || '', md5:this.headers['content-md5'] || '', amazonHeaders:aws.canonicalizeHeaders(this.headers)
        }
    ))
...
Request.prototype.oauth = function (_oauth) {
    ...
    this.headers.Authorization =
        'OAuth ' + Object.keys(oa).sort().map(function (i) {
