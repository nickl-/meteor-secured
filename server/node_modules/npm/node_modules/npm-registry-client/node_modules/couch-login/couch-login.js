...
{ get:makeReq('GET'), del:makeReq('DELETE'), put:makeReq('PUT', true), post:makeReq('POST', true), login:login, logout:logout, decorate:decorate, changePass:changePass, signup:signup, deleteAccount:deleteAccount, anon:anon, anonymous:anon, valid:valid
}
...
function makeReq(meth, body, f) {
    return function madeReq(p, d, cb) {
        assert(this instanceof CouchLogin)
...
function signup(auth, cb) {
    ...
    var newSalt = crypto.randomBytes(30).toString('hex')
        ...
        , user = { _id:'org.couchdb.user:' + auth.name, name:auth.name, roles:[], type:'user', password_sha:newSha, salt:newSalt, date:new Date().toISOString() }
