...
function stripData(data) {
    return { name:data.name, description:npm.config.get("description") ? data.description : "", maintainers:(data.maintainers || []).map(function (m) {
        return "=" + m.name
    }), url:!Object.keys(data.versions || {}).length ? data.url : null, keywords:data.keywords || [], time:data.time
        && data.time.modified
...
function prettify(data, args) {
    ...
    var longest = []
        ...
        , sortFields = { name:0, description:1, author:2, date:3, keywords:4 }
